...
- **Lifecycle**: a `@Script` class implements the **required** `onStart()`, `onUpdate(float deltaTime)`, `onDestroy()` (the mType `@Script` validator enforces these three plus a default constructor). It may also implement the **optional** hooks `onLateUpdate(float deltaTime)`, `onFixedUpdate(float deltaTime)`, `onEnable()`, `onDisable()` — the engine calls them only if present. Frame order is `onUpdate` → engine Navmesh/Transforms → `onLateUpdate`, so `onLateUpdate` reads this frame's post-movement state (`deltaTime` is the same scaled gameplay delta as `onUpdate`). Get the owning entity with `Entity::self()`.
    - **Nav-paced follower rule**: any per-frame follower of a nav-paced unit — selection rings, health bars, minimap blips, attached FX, locomotion params — must read the unit's position/velocity in `onLateUpdate`, not `onUpdate`, or it trails a frame behind the rendered mesh. See `controllers/SoldierCombatController.mt` (locomotion `Speed` from `Navmesh::getVelocity`) and `controllers/UnitSelectionController.mt` (selection-ring position sync) for the established pattern.
